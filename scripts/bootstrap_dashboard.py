...
def chart_specs() -> tuple[ChartSpec, ...]:
    ...
    return (
        ...
        ChartSpec("Hourly Pickup Demand", "hourly_demand", "dist_bar",
                  {**no_time, "groupby": ["pickup_hour"],
                   "metrics": [metric("trip_count", "Trips")]}),
        ChartSpec("Pickup Trips by Borough", "pickup_zone_performance", "dist_bar",
                  {**no_time, "groupby": ["borough"],
                   "metrics": [metric("trip_count", "Trips")]}),
        ChartSpec("Top Pickup Zones", "pickup_zone_performance", "dist_bar",
                  {**no_time, "groupby": ["borough", "zone"], "row_limit": 10,
                   "order_desc": True, "metrics": [metric("trip_count", "Trips")]}),
        ChartSpec("Payment Type Trips", "payment_type_summary", "dist_bar",
                  {**no_time, "groupby": ["payment_type"],
                   "metrics": [metric("trip_count", "Trips")]}),
    )
